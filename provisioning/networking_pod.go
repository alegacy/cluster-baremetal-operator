...
func secretVersionAnnotation(kubeClient kubernetes.Interface, namespace, secretType, secretName string) map[string]string {
	secret, err := kubeClient.CoreV1().Secrets(namespace).Get(context.Background(), secretName, metav1.GetOptions{})
	if err != nil {
		klog.V(3).Infof("secretVersionAnnotation: unable to get secret %s/%s: %v", namespace, secretName, err)
		return nil
	}
...
func EnsureIronicNetworkingDeployment(info *ProvisioningInfo) (updated bool, err error) {
	...
	maps.Copy(podAnnotations, secretVersionAnnotation(info.Client, info.Namespace, "switch-configs", switchConfigsSecretName))
	maps.Copy(podAnnotations, secretVersionAnnotation(info.Client, info.Namespace, "switch-credentials", switchCredentialsSecretName))
...
func DeleteIronicNetworkingDeployment(info *ProvisioningInfo) error {
	return client.IgnoreNotFound(info.Client.AppsV1().Deployments(info.Namespace).Delete(context.Background(), ironicNetworkingDeploymentName, metav1.DeleteOptions{}))
}
...
func DeleteIronicNetworkingService(info *ProvisioningInfo) error {
	return client.IgnoreNotFound(info.Client.CoreV1().Services(info.Namespace).Delete(context.Background(), ironicNetworkingServiceName, metav1.DeleteOptions{}))
}
