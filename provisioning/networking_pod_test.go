...
func TestCreateContainerIronicNetworking(t *testing.T) {
	...
	// No health probes: the JSON-RPC server requires TLS+auth so neither TCP
	// nor HTTP probes can check it without causing TLS handshake errors or
	// 401 failures. The single-replica Recreate deployment restarts crashed
	// containers automatically, making probes unnecessary.
...
func TestSecretVersionAnnotation(t *testing.T) {
	...
	t.Run("returns nil when secret does not exist", func(t *testing.T) {
		...
		ann := secretVersionAnnotation(kubeClient, namespace, "switch-configs", switchConfigsSecretName)
		assert.Nil(t, ann)
	})
	...
	t.Run("hash changes when secret data changes", func(t *testing.T) {
		...
		ann1 := secretVersionAnnotation(kubeClient1, namespace, "switch-configs", switchConfigsSecretName)
		...
		ann2 := secretVersionAnnotation(kubeClient2, namespace, "switch-configs", switchConfigsSecretName)
	...
	t.Run("hash is deterministic with multiple keys", func(t *testing.T) {
		...
		ann1 := secretVersionAnnotation(kubeClient, namespace, "switch-configs", switchConfigsSecretName)
		ann2 := secretVersionAnnotation(kubeClient, namespace, "switch-credentials", switchCredentialsSecretName)
		assert.Equal(t,
			ann1["baremetal.openshift.io/switch-configs-version"],
...
func TestEnsureIronicNetworkingDeploymentSecretAnnotations(t *testing.T) {
	...
	for _, ann := range []map[string]string{
		secretVersionAnnotation(info.Client, info.Namespace, "switch-configs", switchConfigsSecretName),
		secretVersionAnnotation(info.Client, info.Namespace, "switch-credentials", switchCredentialsSecretName),
	} {
